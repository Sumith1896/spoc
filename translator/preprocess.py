...
def check_existing_pt_files(opt):
    ...
    for ext in ('pt', 'gz'):
        pattern = opt.save_data + '.{}*.' + ext
        for t in ['train', 'valid', 'vocab']:
            path = pattern.format(t)
            if glob.glob(path):
                sys.stderr.write("Please backup existing file: %s, "
...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader, opt):
    ...
    ################ MODIFIED ################
    shard_pairs = get_shard_pairs(src, tgt, opt.shard_size)
    dataset_paths = []
    filter_pred = None      # Avoid dropping examples
    ##########################################
    for i, (src_shard, tgt_shard) in enumerate(shard_pairs):
        assert len(src_shard) == len(tgt_shard)
        logger.info("Building shard %d." % i)
        dataset = inputters.Dataset.from_raw(
            fields,
        ...
        data_path = "{:s}.{:s}.{:d}.gz".format(opt.save_data, corpus_type, i)
        dataset_paths.append(data_path)

        logger.info(" * saving %sth %s data shard to %s."
                    % (i, corpus_type, data_path))

        dataset.save_jsonl(data_path)
