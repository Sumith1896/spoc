    ...
    def _compute_loss(self, batch, output, target, copy_attn, align):
        ...
        Args:
            ...
            output: the predict output from the model.
                shape: (seq_len, batch_size, output_embed_dim)
            target: the validate target to compare output with.
                shape: (seq_len, batch_size); type = int
            copy_attn: the copy attention value.
                shape: (seq_len, batch_size, extra_vocab_size)
        ...
        # (seq_len, batch_size, augmented_vocab_size)
        ...
        target_data = orig_target.clone()
        unk = self.criterion.unk_index
        correct_mask = (target_data == unk) & (orig_align != unk)
        offset_align = orig_align[correct_mask] + len(self.tgt_vocab)
        target_data[correct_mask] += offset_align

        # Compute sum of perplexities for stats
        #stats = self._stats(loss.sum().clone(), scores_data, target_data)
        stats = self._batch_stats(loss.sum().clone(), scores_data, target_data)
        ##########################################
