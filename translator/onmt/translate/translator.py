...
class Translator(object):
    ...
    def translate(
        ...
        ################ MODIFIED ################
        data = inputters.Dataset.from_raw(
            self.fields,
    ...
    def _decode_and_generate(
        ...
        if not self.copy_attn:
            ...
            if batch_offset is None:
                ...
                # Make it (batch_size, tgt_len, dynamic_vocab_size)
                scores = scores.view(-1, batch.batch_size, scores.size(-1)).transpose(0, 1)
            else:
                # Make it (batch_size, beam_size, dynamic_vocab_size)
    ...
    def _score_target(self, batch, memory_bank, src_lengths,
        ...
        ################ MODIFIED ################
        # Debug print all non-epsilon probs:
        #with open('debug', 'w') as fout:
        #    for ii in range(log_probs.shape[0]):
        #        for jj in range(log_probs.shape[1]):
        #            for kk in range(log_probs.shape[2]):
        #                if log_probs[ii, jj, kk] > -10:
        #                    print(
        #                        ii, jj, kk,
        #                        self._tgt_vocab.itos[kk] if kk < len(self._tgt_vocab) else '?????',
        #                        log_probs[ii, jj, kk].item(),
        #                        file=fout,
        #                    )
        #            print(file=fout)
        # gold: (tgt_len - 1, batch_size, 1)
        gold = tgt[1:]
        if self.copy_attn:
            # align: (tgt_len - 1, batch_size, 1)
            align = batch.alignment[1:].unsqueeze(2)
            # If gold = 0 and align != 0, replace gold with align
            copy_idxs = (gold == self._tgt_unk_idx) * (align != self._tgt_unk_idx)
            gold[copy_idxs] = align[copy_idxs] + self._tgt_vocab_len
        ##########################################
